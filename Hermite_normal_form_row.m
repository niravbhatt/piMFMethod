...
U = eye(m,m); % initialize U to the identity matrixj,
r = min(m,n);
p = 0; % the location of pivot
for i = 1:r;
    S = find((H((p+1):m,i)) ~= 0); % Find the nonzero entries in row i
    if ~isempty(S)
        p = p+1;
        FINISHED = 0;
        while ~FINISHED
            S = p-1 + find((H(p:m,i)) ~= 0);
            [dummy k] = min(abs(H(S,i))); % find the smallest non-zero entry
            if S(k) ~= p
                index = 1:m; index(p) = S(k); index(S(k)) = p;
            ...
            if isempty(find( H((p+1):end,i) ~= 0) )
                FINISHED = 1;
                if H(p,i) < 0 % flip the sign of H(i,;) if necessary
                    H(p,:) = -H(p,:);
                    U(p,:) = -U(p,:);
                end
